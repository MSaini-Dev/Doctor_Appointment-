...
function isHoliday(date, settings) {
  ...
  return (
    ...
    settings.weeklyHolidays.includes(dayName)
  );
}
app.get("/", (req, res) => {
...
app.post("/weekly-holidays", async (req, res) => {
  ...
  if (!validDays.includes(day)) {
    return res
      .status(400)
      .json({ error: "Invalid day. Must be one of: " + validDays.join(", ") });
  }
  ...
  if (!settings.weeklyHolidays.includes(day)) {
    ...
    res.json({
      success: true,
      message: `${day} added as weekly holiday and existing sessions deleted.`,
    });
  } else {
...
app.get("/sessions", async (req, res) => {
  ...
  const sessions = await Session.find({ date: { $gte: today } }).sort({
    date: 1,
    checkin: 1,
  });
  res.json(sessions);
  ...
  try {
    ...
    if (isHoliday(date, settings)) {
      return res.status(403).json({
        error: `Cannot create session on ${date}. This date is marked as a holiday or falls on ${getDayName(
          date
        )} which is a weekly holiday.`,
      });
    }
    ...
    if (existing) {
      return res
        .status(400)
        .json({ error: "Session already exists for this date/time." });
    }
...
app.delete("/sessions/:id", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Session and associated patients deleted.",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/patient", async (req, res) => {
  try {
    ...
    if (!name || !phone || !sessionId) {
      return res
        .status(400)
        .json({ error: "Name, phone and sessionId required" });
    }
    ...
    if (session.currentToken >= session.totalTokens) {
      return res
        .status(400)
        .json({ error: "No tokens available for this session" });
    }
  ...
  try {
    ...
    if (!["pending", "complete", "cancelled"].includes(status)) {
      return res
        .status(400)
        .json({
          error: "Invalid status. Must be pending, complete, or cancelled.",
        });
    }
